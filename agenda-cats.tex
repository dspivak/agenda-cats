...
For a fixed universe $u$, the ($u$-) coproduct completion of $\cat{C}\op$, which we'll call its \emph{menu category} $\mn_u(\cat{C})$, is a very rich construction. Applying it successively to the empty category, one obtains:
\[
...
\]
however, applying it twice does give a monad, namely the free $u$-distributive category on $\cat{C}$
\[
\fd_u(\cat{C})\cong\mn_u(\mn_u(\cat{C}))
\]
Here $\fd_u(\cat{C})$ has $u$-ary coproducts and $u$-ary products that distribute over them, and it is the free such thing containing a copy of $\cat{C}$. In this way, we can very roughly think of $\mn$ as a kind of ``square root'' of $\fd$. 
